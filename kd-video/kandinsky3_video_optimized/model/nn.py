...
(https://github.com/ai-forever/KandinskyVideo/blob/main/video_kandinsky3/model/nn.py)
"""
...
class TemporalAttention(nn.Module):
    ...
    def forward(self, x):
        query = rearrange(
            self.to_query(x), "b t n (h d) -> b n h t d", h=self.num_heads
        )
        key = rearrange(self.to_key(x), "b t n (h d) -> b n h t d", h=self.num_heads)
        value = rearrange(
            self.to_value(x), "b t n (h d) -> b n h t d", h=self.num_heads
        )

        attention_matrix = (
            einsum("b n h i d, b n h j d -> b n h i j", query, key) * self.scale
        )
        attention_matrix = attention_matrix.softmax(dim=-1)
