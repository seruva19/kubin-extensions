...
(https://github.com/ai-forever/KandinskyVideo/blob/main/kandinsky_video/model/diffusion.py)
"""
...
class BaseDiffusion:
    ...
    def p_sample_loop_image(
        ...
        for time in tqdm(sampler.timesteps):
            ...
            large_x = torch.cat([large_x, large_condition, large_mask], dim=1).to(
                context.dtype
            )
            ...
            pred_noise = (
                guidance_weight + 1.0
            ) * pred_noise - guidance_weight * uncond_pred_noise
    ...
    def p_sample_loop(
        ...
        noise_aug_time = noise_augmentation * torch.ones(
            size=(shape[0],), dtype=torch.int32, device=device
        )
        motion_score = motion_score * torch.ones(
            size=(shape[0],), dtype=torch.int32, device=device
        )

        noise = torch.rand_like(key_frame)
        key_frame = key_frame.long()
        key_frame = sampler.add_noise(
            key_frame, noise, torch.tensor([noise_augmentation]).long()
        )
        condition = torch.zeros_like(x)
        mask = torch.zeros_like(x[:, :1])
        ...
        large_condition = torch.cat(
            [condition, condition, torch.zeros_like(condition)], dim=0
        )
        ...
        large_context_mask = torch.cat(
            [context_mask, uncondition_context_mask, uncondition_context_mask]
        )
        ...
        for time in tqdm(sampler.timesteps):
            ...
            pred_noise, pred_noise_I, uncond_pred_noise = torch.chunk(
                pred_large_noise, 3
            )
            pred_noise = (
                (1 - guidance_weight_image) * uncond_pred_noise
                + (guidance_weight_image - guidance_weight) * pred_noise_I
                + guidance_weight * pred_noise
            )
    ...
    def p_sample_loop_interpolation(
        ...
        skip_frames = skip_frames * torch.ones(
            size=(num_predicted_groups,), dtype=torch.int32, device=device
        )
        perturbation_time = perturbation_time * torch.ones(
            size=(num_predicted_groups,), dtype=torch.int32, device=device
        )
        ...
        left_base_frames = torch.cat(
            [left_base_frames, torch.zeros_like(left_base_frames)]
        )
        right_base_frames = torch.cat(
            [right_base_frames, torch.zeros_like(right_base_frames)]
        )
        ...
        for time in tqdm(self.interpolation_sampler.timesteps):
            ...
            pred_prev_sample = self.interpolation_sampler.step(
                pred_noise, time, x
            ).prev_sample
