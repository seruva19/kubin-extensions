...
(https://github.com/ai-forever/KandinskyVideo/blob/main/kandinsky_video/model/unet_interpolation.py)
"""
...
class TemporalConv3D(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.temporal_projection = nn.Conv3d(
            in_channels, out_channels, (3, 1, 1), padding=(1, 0, 0)
        )
    ...
    def forward(self, x, bs, temporal_mask):
        ...
        self.temporal_merge.data = self.temporal_merge.data.to(dtype=torch.float16)
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.temporal_post_upsample = set_default_layer(
            new_params
            and exist(up_resolution)
            and up_resolution
            and not time_downsample,
            TemporalConv3D,
            (out_channels, out_channels),
            {},
            layer_2=Identity,
        ...
        self.temporal_projection_new = set_default_layer(
            temporal, TemporalConv3D, (out_channels, out_channels), {}, layer_2=Identity
        )
        ...
        self.resnet_blocks = nn.ModuleList(
            [
                ...
                for (
                    in_channel,
                    out_channel,
                ), kernel_size, up_resolution, temporal in zip(
...
class TemporalAttentionBlock(nn.Module):
    def __init__(
        ...
        self.temporal_merge1 = torch.nn.Parameter(torch.tensor([0.0]))
        self.temporal_merge2 = torch.nn.Parameter(torch.tensor([0.0]))
    ...
    def forward(self, x, time_embed, temporal_mask, bs):
        ...
        self.temporal_merge1.data = self.temporal_merge1.data.to(dtype=torch.float16)
        ...
        self.temporal_merge2.data = self.temporal_merge2.data.to(dtype=torch.float16)
...
class DownSampleBlock(nn.Module):
    ...
    def __init__(
        ...
        self.resnet_attn_blocks = nn.ModuleList(
            [
                ...
                for (in_channel, out_channel), up_resolution, time_downsample in zip(
                    hidden_channels, up_resolutions, time_downsample_blocks
                )
            ]
        )
    ...
    def forward(
        ...
        x = self.self_attention_block(
            x, time_embed, temporal_embed=temporal_embed, bs=bs
        )
    ...
    def __init__(
        ...
        self.resnet_attn_blocks = nn.ModuleList(
            [
                ...
                for (in_channel, out_channel), up_resolution, time_downsample in zip(
                    hidden_channels, up_resolutions, time_upsample_blocks
                )
            ]
        )
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.merge_perturbation_time_with_diffusion_time = (
            merge_perturbation_time_with_diffusion_time
        )
        ...
        if (
            merge_perturbation_time_with_context and use_perturbation_time_embed
        ):  # merge with context
            self.time_embed_context_merge = torch.nn.Parameter(torch.tensor([0.0]))
    ...
    def forward(
        ...
        if perturbation_time is None:
            perturbation_time = (
                torch.zeros(size=(context.shape[0],), device=context.device) + 25
            )
        if skip_frames is None:
            skip_frames = (
                torch.zeros(
                    size=(context.shape[0],), dtype=torch.int32, device=context.device
                )
                + 1
            )
        ...
        if (
            self.use_perturbation_time_embed
            and self.merge_perturbation_time_with_diffusion_time
        ):
            perturbation_time_embed = self.perturbation_to_time_embed(perturbation_time)

            self.time_embed_merge = nn.Parameter(
                self.time_embed_merge.to(torch.float32)
            )

            time_embed = (
                self.time_embed_merge * perturbation_time_embed
                + (1 - self.time_embed_merge) * time_embed
            )

        if (
            self.use_perturbation_time_embed
            and self.merge_perturbation_time_with_context
        ):
        ...
        for level, down_sample in enumerate(self.down_samples):
            x = down_sample(
                x, time_embed, context, context_mask, temporal_embed, bs, temporal_mask
            )
