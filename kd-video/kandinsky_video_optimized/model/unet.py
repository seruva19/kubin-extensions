...
(https://github.com/ai-forever/KandinskyVideo/blob/main/kandinsky_video/model/unet.py)
"""
...
class TemporalResNetBlock(nn.Module):
    ...
    def __init__(
        self, num_channels, time_embed_dim, norm_groups=32, compression_ratio=2
    ):
        super().__init__()
        kernel_sizes = [1, 3, 3, 1]
        paddings = [0, 1, 1, 0]
...
class TemporalAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.temporal_norm = ConditionalGroupNorm(
    ...
    def forward(self, x, time_embed, num_frames):
        ...
        out = rearrange(
            out, "(b t) c h w -> b t (h w) c", h=height, w=width, t=num_frames
        )
        out = self.temporal_attention(out)
        out = rearrange(
            out, "b t (h w) c -> (b t) c h w", h=height, w=width, t=num_frames
        )
        x = x + out
...
class DownSampleBlock(nn.Module):
            ...
            [
                nn.ModuleList(
                    [
                        ...
                        TemporalResNetBlock(
                            out_channel, time_embed_dim, groups, compression_ratio
                        ),
                        TemporalResNetBlock(
                            out_channel, time_embed_dim, groups, compression_ratio
                        ),
    ...
    def forward(
        ...
        if exist(temporal_embed):
            x = self.temporal_attention_block(
                x, time_embed + temporal_embed, num_frames
            )
            ...
            [
                nn.ModuleList(
                    [
                        ...
                        TemporalResNetBlock(
                            in_channel, time_embed_dim, groups, compression_ratio
                        ),
                        TemporalResNetBlock(
                            in_channel, time_embed_dim, groups, compression_ratio
                        ),
...
class UNet(nn.Module):
    ...
    def forward(
        ...
        for level, down_sample in enumerate(self.down_samples):
            x = down_sample(
                x, time_embed, context, context_mask, temporal_embed, num_frames
            )
