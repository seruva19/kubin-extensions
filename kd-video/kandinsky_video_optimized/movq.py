...
(https://github.com/ai-forever/KandinskyVideo/blob/main/kandinsky_video/movq.py)
"""
...
class ResnetBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        add_conv=False,
        temporal=False,
        temporal_kernel_size=(3, 3, 3),
        temporal_conv_padding=(1, 1, 1),
        use_3d_conv=False,
        flicker_augmentation=False
    ):
        super().__init__()
        ...
        if temporal:
            ...
            self.temporal_projection1 = nn.Conv3d(
                out_channels,
                out_channels,
                temporal_kernel_size,
                padding=temporal_conv_padding,
            )

            self.temporal_merge2 = torch.nn.Parameter(torch.tensor([0.0]))
            self.temporal_projection2 = nn.Conv3d(
                out_channels,
                out_channels,
                temporal_kernel_size,
                padding=temporal_conv_padding,
            ...
            if (
                flicker_augmentation
            ):  # I added it here, so that it can be trained with temporal layers
    ...
    def forward(self, x, temb, zq=None, num_frames=None, aug_emb=None):
        ...
        if num_frames is not None and self.temporal:
            ...
            self.temporal_merge1.data = self.temporal_merge1.data.to(
                dtype=torch.float32
            )
        ...
        if num_frames and self.temporal:
            ...
            self.temporal_merge2.data = self.temporal_merge2.data.to(
                dtype=torch.float16
            )
...
class TemporalResnetBlock(nn.Module):
    def __init__(
        self, *, in_channels, out_channels=None, dropout, zq_ch=None, add_conv=False
    ):
        super().__init__()
...
class MOVQDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        add_conv=False,
        temporal=False,
        temporal_mid_blocks=True,
        temporal_kernel_size=(3, 3, 3),
        temporal_conv_padding=(1, 1, 1),
        flicker_augmentation=False,
        build_3D_decoder=False,
        temporal_attention_block=False,
        temporal_blocks=False,
        **ignorekwargs
    ):
        super().__init__()
        ...
        if self.temporal_attention_block:
            self.mid.temporal_attn_1 = TemporalAttnBlock(
                block_in, zq_ch, add_conv=add_conv
            )
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            for i_block in range(self.num_res_blocks + 1):
                ...
                if curr_res in attn_resolutions:
                    ...
                    if self.temporal_attention_block:
                        temporal_attn.append(
                            TemporalAttnBlock(block_in, zq_ch, add_conv=add_conv)
                        )
